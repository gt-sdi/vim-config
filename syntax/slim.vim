...
"     unlet! b:current_syntax
"
"     syn match slimBegin  "^\s*\(&[^= ]\)\@!"
"     nextgroup=slimTag,slimClassChar,slimIdChar,slimRuby
"
"     syn region  rubyCurlyBlock start="{" end="}" contains=@slimRubyTop
"     contained
"     syn cluster slimRubyTop    add=rubyCurlyBlock
"
"     syn cluster slimComponent
"     contains=slimClassChar,slimIdChar,slimWrappedAttrs,slimRuby,slimAttr,slimInlineTagChar
"
"     syn keyword slimDocType        contained html 5 1.1 strict frameset
"     mobile basic transitional
"     syn match   slimDocTypeKeyword "^\s*\(doctype\)\s\+"
"     nextgroup=slimDocType
"
"     syn match slimTag           "\w\+"         contained
"     contains=htmlTagName nextgroup=@slimComponent
"     syn match slimIdChar        "#{\@!"        contained nextgroup=slimId
"     syn match slimId            "\%(\w\|-\)\+" contained
"     nextgroup=@slimComponent
"     syn match slimClassChar     "\."           contained nextgroup=slimClass
"     syn match slimClass         "\%(\w\|-\)\+" contained
"     nextgroup=@slimComponent
"     syn match slimInlineTagChar "\s*:\s*"      contained
"     nextgroup=slimTag,slimClassChar,slimIdChar
"
"     syn region slimWrappedAttrs matchgroup=slimWrappedAttrsDelimiter
"     start="\s*{\s*" skip="}\s*\""  end="\s*}\s*"  contained
contains=slimAttr nextgroup=slimRuby
